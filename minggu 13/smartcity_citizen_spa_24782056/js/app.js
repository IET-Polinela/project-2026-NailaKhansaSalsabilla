...
async function loadSummaryStats() {
    try {
        ...
        document.getElementById("statDraft").textContent = reports.filter(report => report.status === "DRAFT").length;
        document.getElementById("statReported").textContent = reports.filter(report => report.status === "REPORTED").length;
        document.getElementById("statVerified").textContent = reports.filter(report => report.status === "VERIFIED").length;
        document.getElementById("statProgress").textContent = reports.filter(report => report.status === "IN_PROGRESS").length;
        document.getElementById("statResolved").textContent = reports.filter(report => report.status === "RESOLVED").length;
    } catch (error) {
        return;
    }
}
